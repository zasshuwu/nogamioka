...
function TitleBar(props: TitleBarProps) {
  ...
  return <nav className="flex justify-between items-center px-4 p-1.5 dark:bg-neutral-900/60 bg-white/60 backdrop-blur-lg">
    ...
    <section className="font-mono w-48 text-end flex justify-end">
      <Link className={`px-2 py-1 mr-2 dark:hover:bg-white/30 dark:bg-white/10 transition-all ease-in-out duration-300 rounded ${sf_pro_display.className}`} href="/home"><BsHouseExclamation></BsHouseExclamation></Link>
      <Link className={`px-2 py-1 mr-2 dark:hover:bg-white/30 dark:bg-white/10 transition-all ease-in-out duration-300 rounded ${sf_pro_display.className}`} href="/uses"><BsWrench></BsWrench></Link>
